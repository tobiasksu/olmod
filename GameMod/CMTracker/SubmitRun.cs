    ...
    internal class CMTracker_PostRun_GameplayManager_DoneLevel
    {
        ...
        private static void Post(string url, Models.Run run)
        {
            var request = new UnityWebRequest(url)
            {
                uploadHandler = new UploadHandlerRaw(Encoding.UTF8.GetBytes(
                    JsonConvert.SerializeObject(run, new JsonSerializerSettings()
                    {
        ...
        private static Models.Run GetPostData()
        {
            ...
            for (int i = 0; i < GameplayManager.m_player_cumulative_damage_by_robot_type.Length; i++)
            {
                ...
                {
                    request.RobotStats.Add(new RobotStat
                    {
                        EnemyTypeId = i,
                        IsSuper = false,
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                {
                    request.PlayerStats.Add(new PlayerStat
                    {
                        IsPrimary = true,
                        WeaponTypeId = i,
